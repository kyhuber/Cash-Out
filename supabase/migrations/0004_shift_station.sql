...
alter table shifts add column station text;
...
create index shifts_workplace_station_idx on shifts (workplace_id, station)
  where station is not null;

-- Only workplaces that opt in are asked for it, so 'station' joins the list of
-- known optional field keys.
alter table workplaces drop constraint optional_fields_known;
